...
export default function App() {
  ...
  const renderListReaders = () => {
    ...
    return (<div><h1 style={S.pageTitle}>List of readers</h1>
      ...
      {/* Upgrade CTA */}
      <div style={{ background: "linear-gradient(135deg,#0B1120,#1A3A5C)", borderRadius: 12, padding: "18px 24px", color: WHITE, marginTop: 20, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div><div style={{ fontSize: 13, fontWeight: 700, marginBottom: 2 }}>Need more room for your team?</div><div style={{ fontSize: 12, color: "#93B8FF" }}>Upgrade to PolicyPro for unlimited readers, 500+ policies, and all jurisdictions.{spendToUpgrade > 0 ? ` You're $${spendToUpgrade} CAD away.` : ""}</div></div>
        <button onClick={() => setShowUpgradePrompt(true)} style={{ ...S.btn(), background: WHITE, color: BLUE, flexShrink: 0, whiteSpace: "nowrap" }}>Upgrade to invite more readers</button>
      </div>
    </div>);
  };
        ...
        {settingsPage === "subscriptions" && (<div>
          ...
          <div style={{ display: "flex", gap: 20, marginBottom: 20 }}>
            ...
            <div style={{ ...S.card, flex: 1 }}>
              ...
              <div style={{ fontSize: 28, fontWeight: 700, color: TEXT_C, marginBottom: 4 }}>${totalSpend + ESSENTIALS_BASE_PRICE} <span style={{ fontSize: 14, fontWeight: 500, color: TEXT_SEC }}>/ $1,699 PolicyPro</span></div>
              ...
              {spendToUpgrade > 0 ? <div style={{ fontSize: 13, fontWeight: 600, color: BLUE, marginTop: 8 }}>${spendToUpgrade} away from PolicyPro value</div> : <div style={{ fontSize: 13, fontWeight: 600, color: GREEN, marginTop: 8 }}>You've reached the PolicyPro threshold!</div>}
  ...
  return (
    <div style={S.app}>
      ...
      {showReaderPaywall && <div style={S.modal} onClick={() => setShowReaderPaywall(false)}><div style={S.modalBox(500)} onClick={e => e.stopPropagation()}><ModalHeader title="Reader limit reached" onClose={() => setShowReaderPaywall(false)} />
        ...
        {/* Free option: remove a reader */}
        <div onClick={() => { setShowReaderPaywall(false); handleNav("list-readers", "readers"); notify("Select a reader to remove and free up a seat"); }} style={{ ...S.card, marginBottom: 12, cursor: "pointer", border: `2px solid ${BORDER}` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><div><div style={{ fontWeight: 700, fontSize: 14, marginBottom: 2 }}>Remove a reader to free up a seat</div><div style={{ fontSize: 12, color: TEXT_SEC }}>Go to your reader list and remove an inactive or unnecessary reader.</div></div><span style={{ fontSize: 12, fontWeight: 700, color: GREEN, background: GREEN_BG, padding: "3px 10px", borderRadius: 10 }}>Free</span></div>
        </div>
        ...
        <div style={{ background: "linear-gradient(135deg,#0B1120,#1A3A5C)", borderRadius: 10, padding: "16px 20px", color: WHITE, cursor: "pointer" }} onClick={() => { setShowReaderPaywall(false); setShowUpgradePrompt(true); }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><div><div style={{ fontWeight: 700, fontSize: 14, marginBottom: 2 }}>Upgrade to PolicyPro®</div><div style={{ fontSize: 12, color: "#93B8FF" }}>Unlimited readers, 500+ policies, all jurisdictions. $1,699/yr.</div></div><ArrowUpRight size={18} color="#93B8FF" /></div>
        </div>
